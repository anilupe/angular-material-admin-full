...
import { Tiempo } from 'src/app/shared/models/tiempo';
...
export class TiempoComponent implements OnInit {
  ...
  openDialog(interes: any = null): void {
    const dialogRef = this.dialog.open(TiempoModalComponent, {
      width: '400px',
      data: interes,
    });
  ...
  eliminar(id: string) {
    ...
    dialogRef.afterClosed().subscribe(async (result) => {
      if (result) {
        this.tiempoService.delete(id).then(() => this.cargarIntereses());
      }
    });
  }
}
